...
describe('Semicolon-terminated protocol messages', () => {
	...
	test('each message dispatched exactly once', () => {
		const chunks = ['DTH:002100,01;DTH:002101,02;']
		const { received } = simulate(chunks)
...
describe('One message per updateData (new contract)', () => {
	test('no message contains an unmatched semicolon mid-string', () => {
		const { received } = simulate(['DTH:002100,01;DTH:002101,02;DTH:000011,03;'])
		for (const msg of received) {
			// Each message must end with ';' and contain no earlier ';'.
			assert.match(msg, /^[^;]+;$/)
		}
	})
